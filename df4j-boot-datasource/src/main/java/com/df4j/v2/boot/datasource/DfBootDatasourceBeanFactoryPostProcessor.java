...
import com.df4j.v2.base.util.StringUtils;
import com.df4j.v2.base.util.ValidatorUtils;
import com.df4j.v2.boot.properties.DfBootDatasourceProperties;
...
public class DfBootDatasourceBeanFactoryPostProcessor
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        LoggerManage.getSysLogger().debug("开始配置自定义多数据源");
        // 获取配置的对象
        ...
        LoggerManage.getSysLogger().info("配置数据源有：{}", datasources.keySet());
        ...
        for(String datasourceKey: datasources.keySet()){
            ...
            if(!datasource.isEnabled()){
                LoggerManage.getSysLogger().info("数据源[{}]配置为禁用，跳过初始化", datasourceKey);
                continue;
            }
            Map<String, Map<String, Object>> nodes = datasource.getNodes();
            Map<String, String> nodeBeanKeys = new HashMap<>();
            for(String nodeKey: nodes.keySet()){
                LoggerManage.getSysLogger().debug("开始配置{}数据源{}节点", datasourceKey, nodeKey);
                ...
                LoggerManage.getSysLogger().info("配置{}数据源{}节点完成", datasourceKey, nodeKey);
        ...
        LoggerManage.getSysLogger().debug("配置自定义多数据源完成");
    }
    ...
    private DataSource initDataSource(Environment environment, String type, String datasourceKey, String nodeKey){
        ...
        // 如果需要初始化，则初始化数据源
        if(ValidatorUtils.notEmpty(DataSourceType.getDataSourceType(type).getInitMethodName())){
    ...
    private void validProperties(DfBootDatasourceProperties dfBootDatasourceProperties){
        if(ValidatorUtils.isEmpty(dfBootDatasourceProperties.getDefaultKey())){
        ...
        for(String key : datasources.keySet()){
            ...
            if(ValidatorUtils.isEmpty(datasource.getType())){
                throw new DfException("数据源[" + key + "]未配置数据源类型");
            }
            if(ValidatorUtils.isEmpty(datasource.getMaster())){
